  ...
  return (
    ...
    <Fragment>
      <div key={project.id} className='border border-zinc-800 rounded-md p-2 min-w-[275px] md:max-w-[275px] hover:border-zinc-600 duration-100 ease-in-out '>
        <a href={project.live}> 
          <p className='text-md text-textBase font-medium uppercase'>{project.name.length > 25 ? `${project.name.slice(0,25)}...` : project.name} </p>
        </a>

        {project?.video ? (
          <a href={project.live}>  
            <video className='w-full h-full object-cover rounded-md my-4' src={project.video} autoPlay loop muted />
          </a>
        ...
        <div className='flex flex-1 items-center justify-between'>
          <p className='text-lg text-gray-300 '>Technologies
            <span className='block text-sm text-gray-500'>
              {project?.github || project?.playStore ? (
                <> {project.techs.length > 34 ? `${project.techs.slice(0,34)}...` : project.techs} </>
              ) : (
                <> {project.techs}  </>
              )}
            </span>
          ...
          {project?.playStore&&(
            <a href={project.playStore}>
              <motion.div whileTap={{scale: 0.8}}>
                  <IoLogoGooglePlaystore className='text-textBase text-3xl cursor-pointer'/>
              </motion.div>
            </a>
